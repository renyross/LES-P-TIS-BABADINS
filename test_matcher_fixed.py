...
def clean_name(name):
    ...
    name = name.lower()
    prefixes = ["les p'tits babadins de ", "les p'tits babadins d'", "les p'tits bababins de ", "les p’tits babadins de", "les p’tits babadins d'", "les ptits babadins de ", "les ptits babadins d'"]
    for p in prefixes:
...
for line in user_data.split('\n'):
    ...
    if len(parts) >= 1 and parts[0].strip():
        ...
        # fix the weird parsing bug for addresses with numbers
        if address.isdigit() and len(parts) > 3:
...
for ul in user_list:
    ...
    for sc in site_creches:
        ...
        # Strict match or city match
        if short_name == sc_name or (short_name and short_name in sc_name) or (short_name and sc_name in short_name) or (short_name in sc.get("ville", "").lower()):
            found_match = True
            
            site_addr = sc.get("adresse", "").lower()
            user_addr = ul["address"].lower()
            
            # Simple check to see if they are completely different
            user_words = set(re.findall(r'[a-z]{4,}', user_addr))
            site_words = set(re.findall(r'[a-z]{4,}', site_addr))
            
            if user_addr and site_addr and len(user_words) > 0 and len(site_words) > 0:
                if len(user_words.intersection(site_words)) == 0:
                    results.append(f"⚠️ {ul['name']}: L'adresse ne correspond pas.\n   Liste client : {ul['address']}\n   Base de données site : {sc.get('adresse')}")
                    mismatch_count += 1
                    break
                    
            results.append(f"✅ {ul['name']} : Trouvé dans la base de données (ID: {sc.get('id', 'N/A')}).")
            found_count += 1
            break
            
    if not found_match:
        results.append(f"❌ {ul['name']} : INTROUVABLE sur le site web actuellement.")
        not_found_count += 1

print("\n".join(results))
print(f"\nRésumé : {found_count} OK, {mismatch_count} Différents, {not_found_count} Introuvables total {len(user_list)}")
