...
def get_current_data():
    ...
    entries = []
    # Simplified parser for { key: "val", ... }
    # Since it's a JS file, we'll try to find each object
    obj_pattern = re.compile(r'\{([^{}]*?)\}', re.DOTALL)
    for obj_match in obj_pattern.finditer(data_str):
        obj_content = obj_match.group(1)
        entry = {}
        # Find key: "value" or key: 'value' or key: true/false
        key_val_pattern = re.compile(r'(\w+):\s*(?:"([^"]*)"|\'([^\']*)\'|(\w+))')
        for kv_match in key_val_pattern.finditer(obj_content):
...
for u in user_data:
    ...
    for i, c in enumerate(current_data):
        ...
        # Try exact normalized match or containment
        if u_norm == c_norm or u_norm in c_norm or c_norm in u_norm:
            match = c
            match_index = i
            break
        # Try city match + some part of name
        city_norm = normalize(c.get('city', ''))
        if city_norm and city_norm in u_norm:
             match = c
             match_index = i
             break
