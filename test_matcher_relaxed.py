...
def extract_city(address_str):
    ...
    # Try to find exactly what looks like a city name or a zip code
    # e.g., "75014 Paris" -> "Paris"
    match = re.search(r'\b\d{5}\b\s+([A-Za-z\- \']+)', address_str)
    if match:
...
print(f"Total creches on the live site: {len(site_creches)}")
...
for line in user_data.split('\n'):
    ...
    if len(parts) >= 1 and parts[0].strip():
        ...
        user_list.append({
            ...
            "address": address,
            "city_guess": extract_city(address)
        })
...
found_count = 0
for ul in user_list:
    short_name = ul["short_name"]
    city_guess = ul["city_guess"]
    
    found = False
    for sc in site_creches:
        sc_name = clean_name(sc.get("nom", ""))
        sc_city = clean_name(sc.get("ville", ""))
        sc_addr = sc.get("adresse", "").lower()
        
        # Super permissive:
        # Match if the short names intersect, or the city guess is present in site city, or city guess is in site address
        if (short_name and short_name in sc_name) or (sc_name and sc_name in short_name) or \
           (short_name and short_name in sc_city) or (sc_city and sc_city in short_name) or \
           (city_guess and city_guess in sc_city) or (city_guess and city_guess in sc_addr):
            results.append(f"MATCH: {ul['full_name']} -> SITE: {sc.get('nom')} ({sc.get('ville')}, {sc.get('adresse')})")
            found = True
            found_count += 1
            break
            
    if not found:
        results.append(f"MISSING: {ul['full_name']}")

print("\n--- RESULTS ---\n")
print("\n".join(results))
print(f"\nSummary: {found_count} broadly matched out of {len(user_list)}")
