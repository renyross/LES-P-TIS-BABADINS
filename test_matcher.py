...
def clean_name(name):
    # Remove common prefixes to get the core city name
    name = name.lower()
    prefixes = [
        "les p'tits babadins de", "les p'tits bababins de", "les p’tits babadins de",
        "les p'tits babadins d'", "les ptits babadins de", "les p'tits babadins"
    ]
    for p in prefixes:
...
# Read main.js to get crechesData
...
# Better parse of crechesData from main.js
match = re.search(r'const\s+crechesData\s*=\s*(\[.*?\]);', js_content, re.DOTALL)
site_creches = []
if match:
    data_str = match.group(1)
    # Basic conversion to dict list
    # Because writing a full JS parser in basic python is hard, we will extract fields manually
...
print(f"Loaded {len(site_creches)} crèches from main.js")
...
print(f"Loaded {len(user_list)} crèches from user input.")
print("\n--- COMPARISON ---\n")

for ul in user_list:
    short_name = clean_name(ul["name"])
    
    # Try to find a match in the site_creches list
    found_match = False
    for sc in site_creches:
        sc_name = clean_name(sc.get("nom", ""))
        
        # Check if short_name is in sc_name or vice versa
        if short_name and (short_name in sc_name or sc_name in short_name or short_name in sc.get("ville", "").lower()):
            found_match = True
            
            # Compare details
            # If there's an address in both
            site_addr = sc.get("adresse", "").lower()
            user_addr = ul["address"].lower()
            
            issues = []
            
            # Simplistic address comparison, just highlighting major differences if they don't share many words
            if user_addr and site_addr:
                user_words = set(re.findall(r'\w+', user_addr))
                site_words = set(re.findall(r'\w+', site_addr))
                if len(user_words.intersection(site_words)) < 2 and len(user_words)>2 and len(site_words)>2:
                   issues.append(f"Address looks different. Site: '{sc.get('adresse')}' vs User: '{ul['address']}'")
            
            if issues:
                print(f"⚠️ {ul['name']} MATCHED BUT DIFFERS:")
                for i in issues: print(f"   {i}")
            else:
                print(f"✅ {ul['name']} (Matched to {sc.get('nom')})")
            break
            
    if not found_match:
        print(f"❌ NOT FOUND ON SITE: {ul['name']}")
